...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    Новый эндпоинт:
    - принимает CSV-файл
    - возвращает полный набор флагов качества, включая дополнительные эвристики HW03
    """
    start = perf_counter()
    ...
    summary = summarize_dataset(df) # Сводка датасета
    missing_df = missing_table(df) # Таблица пропусков
    flags_all = compute_quality_flags(summary, missing_df) # Эвристики качества (сюда входят твои новые флаги)

    flags_bool = {k: bool(v) for k, v in flags_all.items() if isinstance(v, bool)} # берем только булевы флаги для JSON

    latency_ms = (perf_counter() - start) * 1000.0
    print(f"[quality-flags-from-csv] n_rows={df.shape[0]} n_cols={df.shape[1]} latency_ms={latency_ms:.1f} ms")
